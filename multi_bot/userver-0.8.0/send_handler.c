...
send_handler(struct event_control *lecb)
{
  ...
  if (lecb == NULL) {
    ...
    DEBG(MSG_SEND, "send_handler: lecb->head = %d lecb->tail = %d\n", 
		    lecb->head, lecb->tail);
    t = bogus_regs;
  ...
  saved_tail = lecb->tail;
  consecutive = 0;


  while ((lecb->head < lecb->tail) && (event_processed)) 
  {
    ...
    PRINT_TIME(NOFD, &tnow, &tprev,
       	"send_handler: head = %d tail = %d evt_count = %d",
       	lecb->head, lecb->tail, evt_count);
    ...
    assert(evt_count <= lecb->event_list_size);

    evt = &lecb->eventbuf[lecb->head % lecb->event_list_size ];
#ifdef DEBUG_ON
    verify_evt(evt);
#endif
	...
	DEBG(MSG_SEND, 
	    "send_handler: Processing EVT_SIG\n");
	DEBG(MSG_SEND, 
	    "send_handler: Received event: type = %d id = %d\n",
	...
	PRINT_TIME(NOFD, &tnow, &tprev,
	  ...
	  if (sd < 0) {
		...
		socket_new_conn_off();
	      ...
	      /* if the socket is has data to be read go ahead and read it */
	...
	PRINT_TIME(NOFD, &tnow, &tprev,
	  ...
	  PRINT_TIME(sd, &tnow, &tprev,
		...
		print_event(evt, t);
	      ...
	      PRINT_TIME(sd, &tnow, &tprev,
		  "send_handler: accepts_only = %d should_process_sds = %d", 
	...
	DEBG(MSG_SEND, "send_handler: lecb->head = %d "
	     "lecb->tail = %d lecb->event_list_size %d\n",
	     lecb->head, lecb->tail, lecb->event_list_size);
    ...
    if (event_processed) {
      ...
      ++lecb->head;
      DEBG(MSG_SEND, "send_handler: event processed head now = %d\n",
	  lecb->head);
      /* evt->type= (u32) - 1; */
      send_need_to_check_events = 0;
    } else {
      DEBG(MSG_SEND, 
	  "send_handler: event not processed head still = %d\n",
	  lecb->head);
      PRINT_TIME(NOFD, &tnow, &tprev,
	  "send_handler: event not processed head still = %d\n",
	  lecb->head);
      printf("send_handler: event not processed head still = %d\n",
	  lecb->head);
    ...
    PRINT_TIME(NOFD, &tnow, &tprev,
       	"send_handler: at bottom of loop - head = %d tail = %d",
       	ecb->head, ecb->tail);
  } /* while */
...
get_out:
  ...
  errno = save_errno;
  if (t != bogus_regs) {
    PRINT_TIME(NOFD, &tnow, &tprev, "restore_thread: eip = %p", 
	t->eip);
    PRINT_TIME(NOFD, &tnow, &tprev, "restore_thread: flags = %p", 
	t->flags);
...
num_evts(void)
{
  ...
  if (tmptail == tmphead) {
    ...
    evt_count = (ecb->event_list_size) - tmphead + tmptail - 1;
  }
  PRINT_TIME(NOFD, &tnow, &tprev, "num_evts: returning %d", evt_count);
  return(evt_count);
} /* num_evts */
...
free_ecb_slots(void)
{
  ...
  free_slots = ecb->event_list_size - num_evts();
  PRINT_TIME(NOFD, &tnow, &tprev,
      "free_ecb_slots: returning %d", free_slots);
  return(free_slots);
} /* free_ecb_slots */
