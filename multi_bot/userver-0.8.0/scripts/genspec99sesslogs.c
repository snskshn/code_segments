...
/* #define USE_LONG_SESSIONS */
#define LONG_SESSION_LENGTH        (741)
...
#ifdef USE_LONG_SESSIONS
#define MAX_SESSION    (LONG_SESSION_LENGTH)
#else
#define MAX_SESSION    (15)
#endif /* USE_LONG_SESSIONS */
...
/* CDF (discrete distribution) for various kinds of 
 * files in the SpecWeb fileset. 
 * See SpecWeb user guide for details
 */
static double fileCDF[MAX_FILE+1] = {
0.039, 0.098, 0.186, 0.363, 0.716, 0.834, 0.905, 0.955, 1.0
};

/* PDF (discrete distribution) for various kinds of 
 * files in the SpecWeb fileset 
 */
static double filePDF[MAX_FILE+1] = {
0.039, 0.059, 0.088, 0.177, 0.353, 0.118, 0.071, 0.05, 0.044
};

/* CDF (discrete distribution) for file classes in the SpecWeb fileset.*/
static double classCDF[MAX_CLASS+1] = {
0.35, 0.85, 0.99, 1.0
};

/* CDF (discrete distribution) for the file classes in the SpecWeb fileset*/
static double classPDF[MAX_CLASS+1] = {
0.35, 0.50, 0.14, 0.01
};


#ifdef USE_LONG_SESSIONS
static double sessionPDF[MAX_SESSION+1];
#else
static double sessionPDF[MAX_SESSION+1] = {
0.0, 0.3, 0.0, 0.0, 0.0, 0.0636, 0.0636, 0.0636, 0.0636, 
0.0636, 0.0636, 0.0636, 0.0636, 0.0636, 0.0636, 0.0636
};
#endif /* USE_LONG_SESSIONS */

/* These will eventually point to arrays that will hold the 
 * CDF and PDF respectively. 
 * The size of the table depends on the max_dir parameter, 
 * so we can't allocate arrays a priori 
 */
...
/* Track how many sessions of each length are 
 * actually generated in the workload 
 */
...
void usage()
{
  ...
  printf("-r #   : set/use # as random seed\n");
  printf("-d #   : number of directories in workload (numbered from zero)\n");
  printf("-e     : generate escape sequences to turn timers on and off\n");
...
#if 0
  printf("-l #   : length each session must be # (NOTE: not specweb-like)\n");
#endif
  printf("-m #   : limit the size of the file set to # MB (just below)\n");
  printf("-t #.# : run until distribution tolerances are "
         "within a factor of #.#\n");
 ...
 * These arrays are used to track how many files of each 
 * type and class are generated
...
void init() {
  ...
  if( num_dirs > 0 ) {
    ...
    /* Generate the discrete zipf distribution for the 
...
void writeDist(FILE * sf, double *dist, double *pdf, int size, double epsilon)
{
  ...
  fprintf(sf, "# %5s %8s %10s %10s %10s %10s %5s\n",
          "Index", "Count", "Actual%", "Target%", "Diff", "Diff%", "Met");
  for (i=0; i<size; i++) {
    if (dist == sessionDist) {
    ...
    target = pdf[i] * 100.0;
    if ((dist != sessionDist) || (percent != 0.0)) {
       fprintf(sf, "# %5d %8.0lf %10.6lf %10.6lf %10.6lf %10.6lf %5s\n", 
            i, dist[i], percent, target, 
            fabs(percent - target), 
            (((fabs(percent - target)) / target) * 100.0),
...
int main(int argc, char *argv[])
{
  ...
  while ((ch = getopt(argc, argv, "ch?Hd:m:pr:s:S:t:T:e")) != EOF) {
    switch(ch) {
    case 'c':
      do_clear = 1;
      break;
    ...
    case 'e':
      printf("Generating escape sequences to turn timers on and off\n");
      timer_esc_seq = 1;
      break;
  ...
  if (dist_met && session_dist_met && (session_count > min_sessions)) {
    printf("Program completed: distribution tolerances and minimum "
           "sessions were met\n\n");
  }
  
  if (session_count > max_sessions) {
    printf("Program terminated: max_sessions was exceeded\n\n");
  ...
  snprintf(catCmd, 128, "/bin/cat %s %s > %s; /bin/mv %s %s; /bin/rm -f %s", 
           STATSFILE, logfile, TEMPFILE, TEMPFILE, logfile, STATSFILE);
  // ldbg("catCmd = [%s]\n", catCmd);
  rc = system(catCmd);  
  ...
  if( file <= MAX_FILE && class <= MAX_CLASS ) {
    ...
    for( i = 0; i <= MAX_FILE; i++ ) {
      if (!tolerance_met(fileDist, filePDF, i, epsilon)) {
        ldbg("distribution_add_and_check: doesn't meet file "
    ...
    for( i = 0; i <= MAX_CLASS; i++ ) {
      if (!tolerance_met(classDist, classPDF, i, epsilon))  {
        ldbg("distribution_add_and_check: doesn't meet "
             "class distribution tolerances\n");
        return 0;
      }
    }
    ...
    for( i = 0; i < num_dirs; i++ ) {
      if (!tolerance_met(directoryDist, dirPDF, i, epsilon)) {
        ldbg("distribution_add_and_check: doesn't meet directory "
             "distribution tolerances\n");
        return 0;
      }
    }
...
int tolerance_met(double *dist, double *pdf, int index, double epsilon)
{
  ...
  ldbg("tolerance_met: index = %d dist = %lf pdf = %lf " 
       "value = %lf epsilon = %lf ret_val = %d\n", 
